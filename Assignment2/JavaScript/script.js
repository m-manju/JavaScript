...
// dont give keywords like 'array' to the variable names, it can be given along with other names like numberArray, but try not to use it alone
...
// just use foreach once and do both tasks
...
console.log(
    `\ng. [1, 2, "3", 4, 5, 6, "7"] \n \nCompare this array with the above given array and console only if both items of the array have same data type. (Compare each item of this array with each item of the other array)`
);
...
console.log('\n2. ​Console this(pyramid) star pattern');
const n = 5;
for (let i = 1; i <= n; i += 1) {
